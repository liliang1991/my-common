...
public class CircleQueue<T> implements Iterable<T> {
    ...
    public void foreach(int num, CircleQueue.ForEach<T> forEach, boolean sync) {
        foreach(0, num, forEach, sync);
    }

    public void foreach(int num, CircleQueue.ForEach<T> forEach) {
        foreach(0, num, forEach, false);
    }

    public void foreach(int start, int num, CircleQueue.ForEach<T> forEach) {
...
/**
    ...
    public void foreach(int start, int num, CircleQueue.ForEach<T> forEach, boolean sync) {
        int curIndex = index;
        Object[] curData = data;
